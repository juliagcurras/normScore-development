...
normScore <- function(
  ...
  scoreDF_norm$TotalxItem0 <- scoreDF_norm$Total
  scoreDF_norm[which(rownames(scoreDF_norm) == "Log"), "TotalxItem0"] <- scoreDF_norm[which(rownames(scoreDF_norm) == "Log"),  "TotalxItem0"]*item0
  
  ## Sort ####
  scoreDF_norm <- scoreDF_norm %>% dplyr::arrange(TotalxItem0)
  finalRank <- stats::setNames(scoreDF_norm$TotalxItem0, rownames(scoreDF_norm))
  names(scoreDF_norm) <- c(paste0("Item", 1:6), "Total", "TotalxItem0")
  ...
  if (onlyFinalRank){
    ...
    colnames(score_bootstrap) <- c("Normalization", "Mean normScore", "LL95%", "UL95%")
    score_bootstrap <- score_bootstrap %>%
      dplyr::arrange(`Mean normScore`)
    
    score_bootstrap$`LL95%` <- as.numeric(score_bootstrap$`LL95%`)
    score_bootstrap$`UL95%` <- as.numeric(score_bootstrap$`UL95%`)
    score_bootstrap$`Mean normScore` <- as.numeric(score_bootstrap$`Mean normScore`)
    
    
    #----- Gráfico ####
    pForest <- ggplot2::ggplot(
      score_bootstrap,
      ggplot2::aes(
        x = `Mean normScore`, 
